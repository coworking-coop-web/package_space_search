...
?>
<h1 class="pagetitle"><?php if ($visaOnly != 1){ ?>コワーキングスペース検索<?php } else { ?>コワーキングビザjp検索<?php } ?><small></small></h1>

<section class="mod_pagecontents_sec marginBS ex_clearfix">
<h1 class="sectitle"><?php if ($visaOnly != 1){ ?>コワーキングスペースを検索<?php } else { ?>コワーキングビザjpが利用できるスペースを探そう<?php } ?></h1>
...
<div class="mod_spacelist">
	...
	foreach($spaces as $space) {
		...
		<?php if ($space->visa == 1) { ?><li><img src="<?php echo $icon_visa_src;?>" width="38" height="36" alt="コワーキングビザjp加盟"></li><?php } ?>
		</ul>
		<?php } ?>
		<?php /*
		<?php if ($space->visa == 1) { ?>
		<p class="vasalink ex_opaity"><a href="<?php echo View::url('/visa/visauseform/'); ?>?csID=<?php echo intval($space->csID); ?>">コワーキングビザjpを利用する</a></p>
		<?php } ?>
		*/ ?>
